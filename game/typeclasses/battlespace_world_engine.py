"""
Battlespace world feed: periodic operational pressure in the setting.
Emits mission seeds (kind=battlespace) for JSON mission templates to pick up.
"""
...
class BattlespaceWorldEngine(Script):
    def at_script_creation(self):
        self.key = "battlespace_world_engine"
        self.desc = "Battlespace ambient feed (tick + hourly strong beat) → mission seeds."
    ...
    def _fire_one_pool(self, templates, cadence, now, dedupe_slot, tick_idx):
        ...
        dedupe_key = f"battlespace:{chosen['id']}:{dedupe_slot}"

        if chosen.get("broadcast"):
            enqueue_system_alert(
                severity=chosen["severity"],
                category="battlespace",
                title=chosen["title"],
                detail=chosen["summary"],
                source=chosen.get("source") or "battlespace_world_engine",
        ...
        self.db.stats = stats
        logger.log_info(f"[battlespace_world] cadence={cadence} id={chosen['id']}")
    ...
    def _game_clock_weight_multiplier(self) -> float:
        ...
        return float(snap.get("battlespace_weight") or 1.0)
